...
abstract class DAO {
     ...
     * @param type $customDbConnection Optional database connection to use other
    ...
    public function setSortCriteria($sortCriteria) {
        if (isset($sortCriteria) && $sortCriteria !== '') {
     ...
     * be commited or not.
     * @return boolean TRUE if the data row has been removed successfully
     */
    public function remove($rowID = NULL, $autocommit = TRUE) {
        ...
        $sql = 'DELETE FROM ' . $this->getTableName() . ' ' . $filterClause;
        /* Execute SQL statement */
        $dbConnection = $this->getDbConnection();
        if ($autocommit) {
            $dbConnection->beginTransaction();
