...
$.widget("znetdk.zdkform", {
    ...
    _bindEvents: function() {
        ...
        if (this._html5validation()) { /* HTML5 form validation is supported */
            this.element.find('input:not(:radio),.zdk-listbox,textarea').each(function () {
            ...
            this.element.find('input:radio').each(function () {
                $(this).uniqueId().on('invalid.zdkform_' + $(this).attr('id'), $this, $this._setControlInvalid)
                    .on('puiradiobuttonselectionchange.zdkform_' + $(this).attr('id'), $this, $this._setControlValid);
            });
        }
                ...
                if (inputElement.length) {
                    if (inputElement.attr("type") === "checkbox") {
                        if (inputElement.val() === values[property]) {
                            inputElement.puicheckbox('check', false, true);
                        } else {
                            inputElement.puicheckbox('uncheck', true);
                        }
     ...
     * If the message parameter is empty and an error message is currently displayed
     * then it is hidden.
    ...
    _reset: function(event) {
        ...
        $this.element.find(':checkbox').each(function () {
            if ($(this).is('[checked]')) {
                $(this).puicheckbox('check', false, true);
            } else {
                $(this).puicheckbox('uncheck', true);
            }
        });
    ...
    _submitSucceeded: function(response) {
        ...
        } else if (response !== undefined && response.multi_messages && this.options.msgsuccess) {
            znetdk.multiMessages(response.multi_messages); 
        }
    },
    /**
