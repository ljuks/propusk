...
$(function () {
    $.widget("znetdk.zdkdatatable", $.primeui.puidatatable, {
         ...
         * added to each row of the column and is set as an hyperlink if the
         * 'fieldValueTriggered' property is defined.
         */
        _addIconLinks: function() {
            ...
            if(this.data) {
                ...
                for(var j = 0; j < this.options.columns.length; j++) {
                    var columnOptions = this.options.columns[j];
                    if (columnOptions.icon !== undefined) {
                        iconColumns.push({
                            position:j+1,
                ...
                for(var i = 0; i < iconColumns.length; i++) {
                    ...
                    this.tbody.find('td:nth-child(' + columnPosition + ')').each(function(){
                        ...
                        $(this).prepend(icon);
                        if (iconColumns[i].fieldValueTriggered !== undefined) {
                            $this._handleClickOnIconLinkEvents(icon, iconColumns[i].fieldValueTriggered);
                        }
                    });
                }
            }
         ...
         * @param {jQuery} icon
         * @param {String} fieldValueTriggered Column name containing the value
         * to pass to the 'iconclick' event handler when the icon is clicked on
         */
        _handleClickOnIconLinkEvents: function(icon, fieldValueTriggered) {
            var rowPosition = this._getRowIndex(icon.closest('tr')),
                fieldValue = this._getCellValue(fieldValueTriggered, rowPosition),
                $this = this;
